(function(){
    ...
    QUnit.test("Inner scope variables override outter scope variables.", function(assert){
      ...
      function yay(){
        ...
        assert.equal(peanuts, "roasted"); //Bc it was globally scoped 
      }
      yay();
  
      assert.equal(peanuts, "300"); //It was locally scoped 
    });
    ...
    QUnit.test("Variables created with var in a funtion are re-created each time", function(assert){
      ...
      yay();
      assert.equal(this.counter, undefined); //It was undefined and it never changed 
      yay();
    ...
    QUnit.test("Inner scope can access outer scope", function(assert){
      ...
      assert.equal(yay(), "alphaomega"); // this was + with no spaces  
    });
    ...
    QUnit.test("We can do goofy stuff with outer scope", function(assert){
      ...
      yay(); //This calls the name once 
      assert.equal(name, "greg");
      yay();
      assert.equal(name, "greggreg"); //This calls the name twice 
      yay();
      assert.equal(name,  "greggreggreg"); //This is called 3 times 
    });
  
    QUnit.test("We can pass functions to other functions and then run them.", function(assert){
      var im_outter = 10; //This is Global 
      function yay(){
        im_outter /= 5;  
      }
      ...
      something(yay);
      assert.equal(im_outter, 40); //so in order to get this 10*20= 200/5 
    ...
    QUnit.test("We can get crazy with returns.", function(assert){
      ...
      assert.equal(foo(yay), "hello, this is dog"); //so the + is being used through the function 
    });
